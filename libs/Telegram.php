...
class Telegram {
	 ...
	 * Note: This will method only work if the a�?All Members Are Adminsa�� setting is off in the target group. Otherwise members may only be removed by the group&#39;s creator or by the member that added them.<br/>Values inside $content:<br/>
	 * <table>
	 * <tbody>
	 * <tr>
	 * <td><strong>Parameters</strong></td>
	 ...
	 * <td>String</td>
	 * <td>Optional</td>
	 * <td>Pass the offset that a client should send in the next query with the same text to receive more results. Pass an empty string if there are no more results or if you dona�?t support pagination. Offset length cana��t exceed 64 bytes.</td>
	 * </tr>
	 * <tr>
	 ...
	 * <td>String</td>
	 * <td>Optional</td>
	 * <td>Parameter for the start message sent to the bot when user presses the switch button<br><br><em>Example:</em> An inline bot that sends YouTube videos can ask the user to connect the bot to their YouTube account to adapt search results accordingly. To do this, it displays a a�?Connect your YouTube accounta�� button above the results, or even before showing any. The user presses the button, switches to a private chat with the bot and, in doing so, passes a start parameter that instructs the bot to return an oauth link. Once done, the bot can offer a <a href="https://core.telegram.org/bots/api#inlinekeyboardmarkup"><em>switch_inline</em></a> button so that the user can easily return to the chat where they wanted to use the bot&#39;s inline capabilities.</td>
	 * </tr>
	 * </table>
	 * \param $content the request parameters as array
	 * \return the JSON Telegram's reply
	 ...
	 * If you'd like to make sure that the Webhook request comes from Telegram, we recommend using a secret path in the URL, e.g. https://www.example.com/<token>. Since nobody else knows your bota�?s token, you can be pretty sure ita��s us.
	...
	public function setWebhook($url, $certificate = "") {
		if ($certificate == "") {
			...
			$file = new \CURLFile($certificate);
			$requestBody = array('url' => $url, 'certificate' => $file);
		}
	...
	public function Callback_ID() {
		if (isset($this->data["callback_query"]["id"]))
...
/// Get the number of updates
	...
	public function handle($class, $status)
	{

		if ($this->getUpdateType() == 'callback_query')
		{
			foreach($this->handlers_callback as $value)
			{
		...
		{
			if (($value['state'] == -1 || $value['state'] == $status) && ($value['command'] == "" || $value['command'] == $this->Text()))
			{
				call_user_func(array($class, $value['function']));
