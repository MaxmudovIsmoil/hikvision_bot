...
class Database
{
     ...
     * @return bool False on failure / array Database rows on success
     */
    public function getTasks($date, $time) 
    {
      $rows = array();
  
      $sql = "SELECT u.id, u.firstname, u.chat_id,
              t.id as task_id, t.`name`, ut.remind_time, ut.start_day, ut.end_day 
              FROM `user_task` ut
              LEFT JOIN users u ON u.id = ut.user_id
              LEFT JOIN `task` t ON t.id = ut.task_id
              WHERE '" . $date . "' >= ut.start_day 
                AND '" . $date . "' <= ut.end_day 
